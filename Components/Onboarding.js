...
function ContainerAppear({
  visible = true,
  variant = 'fadeUp', // 'scale' | 'fadeUp' | 'fadeDown' | 'fadeLeft' | 'fadeRight' | 'fade'
  duration = 450,
  delay = 0,
  distance = 16,
  ...
  const slides = useMemo(() => [
    {
      ...
      renderTop: () => (
        <ContainerAppear variant="scale" duration={520} delay={40}>
          {/* рамка с обрезкой, чтобы верхний арт не выползал */}
    ...
    },
    {
      ...
      renderTop: () => (
        <ContainerAppear variant="scale" duration={520} delay={40}>
          {/* Жёсткая рамка с overflow:'hidden' — картинки не заходят за пределы слайда */}
          <View style={{ width, height: height * 0.38, overflow: 'visible', position: 'relative', alignItems: 'center', justifyContent: 'center' }}>
            <Image
              source={DOUBLE_TASK}
              style={{
                position: 'absolute',
            ...
            <Image
              ...
              style={{
                ...
                left: -22, bottom: 8, transform: [{ rotate: '-6deg' }],
              }}
              resizeMode="contain"
            />
            <Image
  ...
  return (
    <View style={styles.fullscreen}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          // ВАЖНО: у всей «страницы» overflow:'hidden', чтобы ничто не вылезало за границы слайда
                ...
                <View style={[styles.dots, { marginBottom: 6 }]}>
                  {slides.map((_, i) => (
                    <View
                      ...
                      style={[
                        styles.dot,
                        { opacity: i === index ? 1 : 0.35, transform: [{ scale: i === index ? 1.1 : 1 }] },
                      ]}
                    />
                  ))}
                </View>
...
const styles = StyleSheet.create({
  ...
  body: { color: '#d7e0ea', fontSize: 15, lineHeight: 22, marginBottom: 22 },
  ...
  buttonText: { color: TEXT, fontSize: 20, fontWeight: '700' },
